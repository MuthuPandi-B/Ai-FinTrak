...
const HomePage = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen">
      {/* Navbar */}
      <header className="bg-blue-600 text-white p-4 text-center">Navbar</header>

      <div className="flex flex-1">
        {/* Dashboard Sidebar - Visible in large screens, toggle in mobile */}
        {showDashboard && (
          <div
            className={`fixed inset-y-0 left-0 w-64 bg-gray-900 text-white p-4 transition-transform duration-300 lg:static lg:w-1/4 ${
              isMobile ? "translate-x-0" : "block"
            }`}
          >
            {isMobile && (
              <button className="lg:hidden bg-red-500 p-2 rounded mb-4" onClick={() => setShowDashboard(false)}>
                Close
              </button>
            )}
            <Dashboard />
        ...
        <main className="flex-1 flex flex-col items-center justify-center space-y-8 p-6">
          ...
          {isMobile && !showDashboard && (
            <button className="bg-blue-600 text-white px-4 py-2 rounded mt-4" onClick={() => setShowDashboard(true)}>
              Open Dashboard
            </button>
          )}
        </main>
